...
Using Azure cognitive services and Python, perform face recogition / detection in real time videos.
  ...
  Then in the next part of the project we´ll export the results to the cloud and start the data engineering process with the data generated by the script, the final architeture will be something like this. The face recognition code will work with Arduino, that is controlling a door´s lock, to grant or deny access based on the models we trainned, when comparing it with the image captured by a IPcam.
